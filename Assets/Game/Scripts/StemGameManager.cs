...
public class StemGameManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        switch (stemGameType)
        {
            case StemGameType.InputSequential:
                if (!TryGetComponent<InputSequential>(out _))
                    gameObject.AddComponent<InputSequential>();
                break;
        }
    }
    ...
    public void AddUst()
    {
        int GuncelUst = PlayerPrefs.GetInt("artis-azalis.Score", 0) + 1;
        PlayerPrefs.SetInt("artis-azalis.Score", GuncelUst);
